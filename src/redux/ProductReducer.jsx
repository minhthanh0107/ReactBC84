...
import { httCilentStoreApi } from "../Settings/setting";
...
export const { getAllProductAction, getProductEditAction } =
...
export const getAllProductActionThunk = async (dispatch, getState) => {
  try {
    ...
    const res = await httCilentStoreApi.get(`/Product`);
...
export const getAllProductEditApiThunk = (idProduct) => {
  return async (dispatch, getState) => {
    try {
      const rootReducer = getState();
      console.log(rootReducer);
      const res = await httCilentStoreApi.get(`/Product/getbyid?id=${idProduct}`);
      //Sau khi có dữ liệu từ api => dispatch lên reducer
